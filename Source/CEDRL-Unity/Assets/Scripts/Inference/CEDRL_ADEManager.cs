...
public class CEDRL_ADEManager : MonoBehaviour
{
    public static CEDRL_ADEManager Instance;

    [Header("Agent Spawning")]
    [Tooltip("CEDRL_Agent가 부착된 에이전트 프리펩")]
    ...
    private Dictionary<int, CEDRL_Agent> spawnedAgents = new Dictionary<int, CEDRL_Agent>();
    ...
    void CheckAndSpawnAgents(int frame)
    {
        ...
        foreach (int id in pendingIds)
        {
            ...
            if (frame >= startFrame && !spawnedAgents.ContainsKey(id))
            {
                ...
                CEDRL_Agent client = go.GetComponent<CEDRL_Agent>();
                if (client != null)
                {
                    int lastF = gtData[id].posByFrame.Keys.Max();
                    Vector3 goalPos = RawToWorld(gtData[id].posByFrame[lastF], pivot);

                    AgentScores defaultScores = new AgentScores { Norm_score = 1.0f };
                    client.SetData(id, _accumulatedSimTime, spawnPos, goalPos, defaultScores, 0f, m_env, null, SceneSetup.Default, false);
                    ...
                    if (groupMap.TryGetValue(id, out List<int> memberIds))
                    {
                        ...
                        {
                            if (mId == id) continue;
                            if (spawnedAgents.TryGetValue(mId, out CEDRL_Agent memberClient))
                            {
    ...
    void SampleAndEvaluate(int frame)
    {
        ...
        {
            CEDRL_Agent agent = spawnedAgents[id];
