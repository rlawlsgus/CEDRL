	...
	public class Agent : IAgent {
		...
		internal struct VO {
			...
			public static VO SegmentObstacle (Vector2 segmentStart, Vector2 segmentEnd, Vector2 offset, float radius, float inverseDt, float inverseDeltaTime) {
				...
				var closestOnSegment = ClosestPointOnSegment(segmentStart, segmentEnd, Vector2.zero);
					...
					if (det1 >= 0 && det2 >= 0) {
						...
						if (Vector2.Dot(p - line1, dir1) > 0 && Vector2.Dot(p - line2, dir2) < 0) {
							if (segment) {
								...
								if (det3 < radius) {
									var closestPointOnLine = (Vector2)ClosestPointOnSegment(segmentStart, segmentEnd, p);
